...
# enrollment token is read from its mounted file and handed to the process in memory only;
# the runner clears it after enrolling and never writes it back to disk. It re-reads the
# FILE (never a retained copy) if it ever has to recover an expired identity.
...
# nears expiry. The bootstrap token is presented again only if that window is MISSED and the
# certificate expires, which renewal cannot repair (it authenticates with the expired cert).
