...
export const DIVERGENCES = [
  ...
  },
  {
    id: "DIV-UI-010",
    kind: "ui",
    subject: "submitting the Slack workspace registration, which needs an agent with a PUBLISHED revision",
    detail:
      "app/integrations resolves the chosen agent's newest PUBLISHED revision on submit and REFUSES an agent that has none, because api/slack_connections.go's vetSlackDefaultPolicy requires agent_revision_id and admission verifies published_at before pinning a run — so registering against a draft would create a workspace that admits nothing, which is today's silent failure with one more step in front of it. The fixture seeds exactly one such agent (agt_01 / seeded-agent-01, revision agrev_2 published), so the two legs that SUBMIT the form run there. A bootstrap compose stack has ZERO agents: `palai up` creates one only when Slack is being wired, which is the very thing this screen exists to do without the CLI — so the real profile's own emptiness is what makes those two legs unrunnable, and it is a genuine property of a fresh stack rather than a fixture convenience. WHAT DOES NOT NARROW, and it is most of the screen: the route, the empty state, the connect dialog opening, all three credential fields being password inputs with autocomplete=new-password and programmatic labels, the form carrying method=post so an unhydrated submit cannot put a token in the URL, and axe over the dialog with every credential control RENDERED. Those four legs run on BOTH profiles and they are the ones that carry the security properties; what needs the fixture is the happy path and the conflict retry, both of which need a run target to exist first. The console's refusal when no published revision exists is itself asserted rather than assumed — it is the arm an operator on a fresh stack actually meets.",
