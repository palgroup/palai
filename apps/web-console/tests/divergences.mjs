...
export const DIVERGENCES = [
  ...
  },
  {
    id: "DIV-UI-009",
    kind: "ui",
    subject: "the MACHINE half of the fleet screen: an enrolled runner, a waiting-room enrolment, and the lifecycle verbs",
    detail:
      "A compose stack enrols NO MACHINES and cannot be made to. A row in `runners` is written only by fleet.Store.Register, which is reached over the RUNNER PLANE — a separate mTLS listener that a real machine dials with a pool key and a CSR (apps/control-plane/internal/fleet) — and no /v1 route creates one: E24's registry surface READS the fleet and MOVES a machine, it cannot put one in. deploy/compose/compose.yaml starts a control plane and a Postgres; it starts no runner, and a runner is a host agent rather than a service this repo composes. So on the real profile GET /v1/runners answers 200 with an EMPTY page and every leg that needs a MACHINE runs on the fixture: the waiting room's row, the admission, the three typed refusals of an approval, cordon/resume/revoke, the lease count the revoke dialog reads, a pool's NON-ZERO queue depth (RunnerGateway.Waiting counts attempts parked for a pool with no free machine, which needs both a run and a machine that is not there), and the concurrency ceiling that is shown on a machine count. WHAT DOES NOT NARROW, and it is most of the screen: the page itself, the pool list, POOL CREATION and the strict switch (E28 T1's routes work against a real control plane and the console drives them there), the pool-key panel, MINTING a key and seeing its value once, REVOKING it, the empty states of the machine and waiting-room panels, all three ceiling sentences, the two mutual scope sentences, and axe over the whole page. The empty state is worth scanning rather than a loss: a first-day operator's fleet screen is exactly this, and a console that answers an empty fleet with whitespace is unreadable. The control plane's half of the machine legs is proven against a REAL Postgres and the REAL shipped router at the component tier — apps/control-plane/internal/execution's strict-enrolment and runner-lifecycle component tests, and E28 T1's crown test drives an enrolment through the public API from a pool that did not exist before it. Re-derived by the sweep from the real stack rather than from this prose: its `runners` collection is EMPTY, which is also why the sweep's item floor does not rise for that route.",
    owner: "compose tier — a runner is a HOST agent, not a composed service; NOT closable from apps/web-console",
  },
];
