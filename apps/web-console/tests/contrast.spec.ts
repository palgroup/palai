...
// THE LANE PALETTE, MEASURED ON THE PAGE (E30 visual identity).
//
// The console's one saturated colour is now the eight journal lanes plus red for a failure, and every one of
// them is a NEW contrast pair. Two things need judging and axe judges neither:
//
//   1. A LANE MARK is a non-text carrier — SC 1.4.11 Non-text Contrast, 3:1 against the surface behind it.
//      It is a <span> with a background and no text, so `color-contrast` never looks at it, and the sweep
//      above only walks `input, select, textarea, button`. Amber is why this leg exists rather than being
//      taken on trust: Radix's step 9, the obvious choice for a solid tick, measures 1.53:1 on this page in
//      light — so the one lane that means "a human is holding this" would have been the one nobody could see.
//   2. A LANE BADGE is TEXT on its own tinted fill — SC 1.4.3, 4.5:1. axe does measure this one, and this
//      leg still asserts it: the badge's fill is set from a custom property, and a property that failed to
//      resolve leaves the word on the fallback surface where it may pass by accident.
//
// scripts/measure-contrast.mjs computes the same pairs from app/globals.css and is the cheap re-run. THIS is
// the one that can fail for the reason that actually happens — an element that does not use the palette.
test("every lane mark and badge carries its floor against the surface it is drawn on", async ({ page }) => {
  // The transcript is the only route that renders all three: the stave, the lane badges, and (on the fixture)
  // seven of the eight lanes. The sessions list is walked too because its activity bar sits on a TABLE ROW,
  // which is a different surface and the one a pointer hover darkens further.
  const transcript = await concreteDynamicPath(page, DYNAMIC_CONSOLE_ROUTES[0]);
  const measured: { route: string; kind: string; lane: string; ratio: number; floor: number }[] = [];
  for (const route of [transcript, "/sessions", "/"]) {
    await page.goto(route);
    await expect(page.locator("main")).toBeVisible();
    await page.waitForLoadState("networkidle");
    const found = await page.evaluate(() => {
      ...
      const out: { kind: string; lane: string; fg: string; bg: string; floor: number }[] = [];
      for (const el of document.querySelectorAll(".strip-mark")) {
        const style = getComputedStyle(el);
        out.push({
          kind: "mark",
          lane: `${el.getAttribute("data-lane") ?? "?"}${el.getAttribute("data-failure") === "true" ? " (failure)" : ""}`,
          fg: style.backgroundColor,
          bg: opaqueBehind(el),
          floor: 3,
        });
      }
      for (const el of document.querySelectorAll(".lane-badge")) {
        const style = getComputedStyle(el);
        const lane = el.getAttribute("data-lane") ?? "?";
        // The word on the badge's own fill (SC 1.4.3) and the leading RULE on that same fill — the rule is
        // the badge's non-colour carrier and is judged as a non-text one.
        out.push({ kind: "badge word", lane, fg: style.color, bg: style.backgroundColor, floor: 4.5 });
        out.push({ kind: "badge rule", lane, fg: style.borderLeftColor, bg: style.backgroundColor, floor: 3 });
      }
      return out;
    });
    for (const f of found) measured.push({ route, kind: f.kind, lane: f.lane, ratio: contrastRatio(f.fg, f.bg), floor: f.floor });
  }

  // THE POSITIVE CONTROL. A palette sweep that found no marks would pass while measuring nothing, which is
  // the empty-haystack shape this file's own dialog arm exists to refuse. The fixture journal draws fourteen
  // frames across seven lanes and the sessions list draws a bar per row that ran.
  expect(measured.length, "no lane mark or badge was found on any route — this sweep is judging an empty page").toBeGreaterThan(20);
  const lanes = [...new Set(measured.map((m) => m.lane))].sort();
  const failing = measured.filter((m) => m.ratio < m.floor);
  const worst = [...measured].sort((a, b) => a.ratio / a.floor - b.ratio / b.floor)[0];
  // eslint-disable-next-line no-console -- the numbers ARE the measurement.
  console.log(
    `LANE PALETTE SWEEP — ${String(measured.length)} mark(s)/badge(s) over 3 route(s), ${String(lanes.length)} lane(s) [${lanes.join(", ")}], ` +
      `${String(failing.length)} under floor; weakest ${worst.kind} ${worst.lane} on ${worst.route} at ${worst.ratio.toFixed(2)}:1 against a floor of ${String(worst.floor)}`,
  );
  expect(
    failing.map((m) => `${m.route} ${m.kind} ${m.lane} at ${String(m.ratio)}:1 (floor ${String(m.floor)})`),
    "a lane's colour is under its floor on the rendered page. The lane palette is the only saturated colour in " +
      "this console, so a lane nobody can see is a lane that carries nothing.",
  ).toEqual([]);
});
