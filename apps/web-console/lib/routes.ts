...
export const CONSOLE_ROUTES: readonly ConsoleRoute[] = [
  ...
  //
  // IT WAS "/integrations" AND IT IS NOT A RENAME (2026-08-03 plan, Task 11). That route wrote
  // `slack_connections`, the registry the control plane's own in-process Slack adapter consumes; this one
  // writes `integration_bots`, a KIND-AGNOSTIC registry the control plane never interprets and a separate
  // relay process reads. One operator intent, one screen — so the old one is gone rather than left beside
  // it, and the shipped in-process path keeps serving the row it already has until Task 14 removes it.
  //
  ...
  // scan covers the empty state — the state a first-day operator meets, and the one the create button sits
  // in.
  { path: "/bots", label: "Bots", group: "Build", readyTestId: "panel-bots", lead: "The bots people talk to this deployment through. Each row is read by a relay process at startup; credentials are sealed into this deployment's secret store and are not written to any machine's disk." },
