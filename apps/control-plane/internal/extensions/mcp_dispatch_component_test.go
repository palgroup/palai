...
func createStdioConnection(t *testing.T, s *Store, project, name string) string {
	t.Helper()
...
func latestRevisionID(t *testing.T, s *Store, project, canonical string) string {
	t.Helper()
	var id string
...
func seedRunWithMCPRider(t *testing.T, s *Store, project, setID string, riders string) string {
	t.Helper()
	...
	mustExec(t, s.pool, `INSERT INTO agent_profiles (id, project_id, name) VALUES ($1,$2,$3)`, profileID, project, profileID)
	mustExec(t, s.pool, `INSERT INTO agent_revisions (id, project_id, profile_id, revision_number, model, published_at, tool_sets, mcp_connections)
	                     VALUES ($1,$2,$3,1,'model-x',clock_timestamp(),$4::jsonb,$5::jsonb)`, arevID, project, profileID, `["`+setID+`"]`, riders)
...
func publishDiscoveredIntoSet(t *testing.T, s *Store, project, connID, canonical string) string {
	t.Helper()
	ctx := context.Background()
	if _, err := s.DiscoverConnection(ctx, project, connID); err != nil {
		t.Fatalf("discover: %v", err)
	}
	revID := latestRevisionID(t, s, project, canonical)
	if _, _, err := s.PublishToolRevision(ctx, project, revID, nil); err != nil {
		t.Fatalf("publish discovered revision: %v", err)
	}
	set, err := s.CreateToolSetRevision(ctx, project, "mcpset", pins(revID, nil))
	if err != nil {
...
func TestMCPCallFlowsThroughStandardDispatch(t *testing.T) {
	...
	runNoRider := seedRunWithMCPRider(t, s, project, setID, `[]`)
...
func TestDiscoveredToolsNamespacedByConnectionNoCollision(t *testing.T) {
	...
	connA := createStdioConnection(t, s, project, "alpha")
	connB := createStdioConnection(t, s, project, "beta")
	setA := publishDiscoveredIntoSet(t, s, project, connA, "mcp.alpha.search")
	setB := publishDiscoveredIntoSet(t, s, project, connB, "mcp.beta.search")

	// Each run pins its own connection's set + rider and resolves ITS namespaced tool.
	runA := seedRunWithMCPRider(t, s, project, setA, `["`+connA+`"]`)
	if tool, found, err := s.LookupTool(ctx, project, runA, "alpha__search"); err != nil || !found {
	...
	runB := seedRunWithMCPRider(t, s, project, setB, `["`+connB+`"]`)
	if _, found, err := s.LookupTool(ctx, project, runB, "beta__search"); err != nil || !found {
	...
	if _, found, _ := s.LookupTool(ctx, project, runA, "beta__search"); found {
...
func TestAnnotationChangeRequiresNewRevisionAndReapproval(t *testing.T) {
	...
	first, err := s.DiscoverConnection(ctx, project, connID)
	if err != nil || len(first.NewRevisions) != 1 {
		t.Fatalf("first discover = %+v err=%v, want one new revision", first, err)
	}
	rev1 := latestRevisionID(t, s, project, "mcp.docs.echo")
	if _, _, err := s.PublishToolRevision(ctx, project, rev1, nil); err != nil {
		t.Fatalf("publish v1: %v", err)
	}

	// Re-discovery with the SAME description writes nothing (idempotent — no manifest churn).
	again, err := s.DiscoverConnection(ctx, project, connID)
	...
	v1, _, _ := s.GetToolRevision(ctx, project, rev1)
	v2, _, _ := s.GetToolRevision(ctx, project, rev2)
	if !v1.Published {
...
func TestMCPDiscoveredToolAdvertisedOnlyWhenPublishedAndPinned(t *testing.T) {
	...
	draftRun := seedRunWithMCPRider(t, s, project, "tsrev_none", `["`+connID+`"]`)
	...
	setID := publishDiscoveredIntoSet(t, s, project, connID, "mcp.docs.echo")
	pubRun := seedRunWithMCPRider(t, s, project, setID, `["`+connID+`"]`)
...
func TestMCPToolTenantIsolated(t *testing.T) {
	...
	connB := createStdioConnection(t, s, projectB, "docs")
	setB := publishDiscoveredIntoSet(t, s, projectB, connB, "mcp.docs.echo")

	// Tenant A run even naming B's set + connection id resolves nothing (the joins are tenant A-pinned).
	runA := seedRunWithMCPRider(t, s, projectA, setB, `["`+connB+`"]`)
