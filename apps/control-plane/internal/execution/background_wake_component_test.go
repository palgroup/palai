...
func newWakeFixture(t *testing.T) *wakeFixture {
	...
	stmts := [][]any{
		{`INSERT INTO workspaces (id, project_id, session_id, state)
		  VALUES ($1,$2,$3,'ready')`, wsID, f.tenant.Project, f.sessionID},
		{`INSERT INTO workspace_allocations (id, workspace_id, project_id, fence, host_path, state)
		  VALUES ($1,$2,$3,1,$4,'active')`, redeliveryID("alloc"), wsID, f.tenant.Project, f.root},
	}
