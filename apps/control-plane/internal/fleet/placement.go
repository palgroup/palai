...
// revision's binding, then the project policy, then the default.
//
// The default is never "no pool". A run with nothing configured anywhere lands in DefaultPoolID, which
// is where every machine in a deployment that has configured nothing also enrols — so the answer for
// today's single-runner install is the same answer it has always had, expressed as a decision instead
// of an absence (§2).
func ResolvePool(req PoolRequest) string {
	for _, candidate := range []string{req.RunPoolID, req.RevisionPoolID, req.PolicyPoolID} {
