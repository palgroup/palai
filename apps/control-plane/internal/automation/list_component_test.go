...
func TestListAgentsAndTriggersTenantScoped(t *testing.T) {
	...
	profileA, err := s.CreateProfile(ctx, projA, "reviewer")
	if err != nil {
		t.Fatalf("create profile A: %v", err)
	}
	if _, err := s.CreateProfile(ctx, projA, "planner"); err != nil {
		t.Fatalf("create profile A2: %v", err)
	}
	for _, m := range []string{`{"model":"m1"}`, `{"model":"m2"}`} {
		if _, err := s.CreateRevision(ctx, projA, profileA, []byte(m)); err != nil {
			t.Fatalf("create revision A: %v", err)
		}
	}
	if _, err := s.CreateProfile(ctx, projB, "b-only"); err != nil {
		t.Fatalf("create profile B: %v", err)
	}

	profiles, err := s.ListProfiles(ctx, projA, ListWindow{Limit: 10})
	if err != nil || len(profiles) != 2 {
		t.Fatalf("ListProfiles(A) err=%v len=%d, want 2 (tenant-scoped)", err, len(profiles))
	}

	revs, err := s.ListRevisions(ctx, projA, profileA, ListWindow{Limit: 10})
	...
	if empty, err := s.ListRevisions(ctx, projA, "aprof_missing", ListWindow{Limit: 10}); err != nil || len(empty) != 0 {
	...
	page1, err := s.ListProfiles(ctx, projA, ListWindow{Limit: 1})
	if err != nil || len(page1) != 1 {
		t.Fatalf("profiles page1 err=%v len=%d, want 1", err, len(page1))
	}
	last := page1[0]
	page2, err := s.ListProfiles(ctx, projA, ListWindow{Limit: 1, AfterCreatedAt: &last.CreatedAt, AfterID: last.ID})
