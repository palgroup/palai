...
func mustCreateQueueConn(t *testing.T, store *QueueStore, project string, in QueueConnectionInput) string {
	t.Helper()
...
func atomicEffectHandler(store *QueueStore, pool *pgxpool.Pool, project, connID string, after queue.Disposition, crash bool) queue.Handler {
	return func(ctx context.Context, m queue.Message) (queue.Disposition, error) {
		...
		fresh, err := store.RecordEffect(ctx, tx, project, connID, m.IdempotencyKey)
		if err != nil {
			return queue.Retry, err
		}
		if fresh {
...
func countScratchEffect(t *testing.T, pool *pgxpool.Pool, project, connID, key string) int {
	t.Helper()
	var n int
...
func countReceipts(t *testing.T, pool *pgxpool.Pool, project, connID, key string) int {
	t.Helper()
	var n int
	if err := pool.QueryRow(storage.ScopeToTenant(context.Background(), project),
...
func TestQueueAdapterRedeliversAfterLostAckSingleEffect(t *testing.T) {
	...
	connID := mustCreateQueueConn(t, store, project,
	...
	// lost-ack window.
	if n, err := q.Consume(ctx, 10, atomicEffectHandler(store, pool, project, connID, queue.Retry, false)); err != nil || n != 1 {
	...
	if n, err := q.Consume(ctx, 10, atomicEffectHandler(store, pool, project, connID, queue.Ack, false)); err != nil || n != 1 {
		t.Fatalf("redelivery Consume = (%d, %v), want (1, nil)", n, err)
	}

	if got := countScratchEffect(t, pool, project, connID, "order-42"); got != 1 {
	...
	if got := countReceipts(t, pool, project, connID, "order-42"); got != 1 {
...
func TestQueueAdapterEffectAtomicWithReceiptOnCrash(t *testing.T) {
	...
	if _, err := q.Consume(ctx, 10, atomicEffectHandler(store, pool, project, connID, queue.Ack, true)); err != nil {
	...
	if got := countReceipts(t, pool, project, connID, "charge-1"); got != 0 {
		t.Fatalf("receipts after rollback = %d, want 0 (the receipt rolled back with the effect)", got)
	}
	if got := countScratchEffect(t, pool, project, connID, "charge-1"); got != 0 {
	...
	if got := countScratchEffect(t, pool, project, connID, "charge-1"); got != 1 {
		t.Fatalf("effect rows after redelivery = %d, want 1 (effect applied exactly once on the retry)", got)
	}
	if got := countReceipts(t, pool, project, connID, "charge-1"); got != 1 {
...
func TestQueueAdapterConcurrentEffectSerializes(t *testing.T) {
	...
	sctx := storage.ScopeToTenant(context.Background(), project)
	...
	for i := 0; i < 2; i++ {
		...
		go func(i int) {
			...
			fresh, err := store.RecordEffect(sctx, tx, project, connID, "race-1")
			if err != nil {
	...
	if got := countScratchEffect(t, pool, project, connID, "race-1"); got != 1 {
		t.Fatalf("committed effect rows = %d, want 1 (exactly one effect under concurrency)", got)
	}
	if got := countReceipts(t, pool, project, connID, "race-1"); got != 1 {
...
func TestQueueOutboxDeliversLosslesslyExactlyOnce(t *testing.T) {
	...
	outbox, err := store.Outbox(ctx, project, connID)
	if err != nil {
	...
	if got := queueDeliveryState(t, pool, project, "result-7", connID); got != "pending" {
	...
	outbox2, err := store.Outbox(ctx, project, connID)
	if err != nil {
	...
	if got := queueDeliveryState(t, pool, project, "result-7", connID); got != "delivered" {
...
func queueDeliveryState(t *testing.T, pool *pgxpool.Pool, project, destKey, connID string) string {
	t.Helper()
	var state string
	if err := pool.QueryRow(storage.ScopeToTenant(context.Background(), project),
