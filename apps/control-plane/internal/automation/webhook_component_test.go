...
func appendEventAt(t *testing.T, pool *pgxpool.Pool, org, project, session string, seq int, journalID int64, typ string) {
	...
	mustExec(t, pool, `SELECT setval(pg_get_serial_sequence('events','journal_id'), (SELECT max(journal_id) FROM events))`)
}

func maxJournalID(t *testing.T, pool *pgxpool.Pool) int64 {
	t.Helper()
	var m int64
	if err := pool.QueryRow(context.Background(), `SELECT COALESCE(max(journal_id),0) FROM events`).Scan(&m); err != nil {
