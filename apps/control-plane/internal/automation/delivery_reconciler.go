...
// Tick runs one sweep: recover stuck-mapped remnants, re-drive/scrub ack'ed inbound remnants (T5),
// admit gate-opened deferred FIFO heads, then arm + mirror run-terminal callbacks (T6). Each step is
// set-based / idempotent, so a re-run is a no-op.
