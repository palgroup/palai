...
func TestRedeliveredTerminalStaysSingleByMonotonicity(t *testing.T) {
	...
	terminalsBefore := h.count(`SELECT count(*) FROM events WHERE session_id=$1 AND project_id=$2 AND type='run.completed.v1'`,
	...
	terminalsAfter := h.count(`SELECT count(*) FROM events WHERE session_id=$1 AND project_id=$2 AND type='run.completed.v1'`,
		sessionID, h.tenant.Project)
	if terminalsAfter != 1 {
