...
func (h *harness) childRunOf(parentRunID string) (runID, responseID string) {
	...
	if err := h.spine.Pool().QueryRow(storage.WithSystemScope(context.Background()),
		`SELECT id, response_id FROM runs WHERE parent_run_id=$1 AND project_id=$2`,
...
func (h *harness) modelOfRun(responseID string) string {
	...
	var model *string
	if err := h.spine.Pool().QueryRow(storage.WithSystemScope(context.Background()),
		`SELECT output->>'model' FROM responses WHERE id=$1 AND project_id=$2`,
...
func (h *harness) childRunsLink(responseID string) []string {
	...
	var raw []byte
	if err := h.spine.Pool().QueryRow(storage.WithSystemScope(context.Background()),
		`SELECT output->'child_runs' FROM responses WHERE id=$1 AND project_id=$2`,
...
func (h *harness) childEffectiveBudget(childRunID string) int {
	...
	var budget *int
	if err := h.spine.Pool().QueryRow(storage.WithSystemScope(context.Background()),
		`SELECT (delegation->'spec'->>'budget')::int FROM runs WHERE id=$1 AND project_id=$2`,
