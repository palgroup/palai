...
func TestLiveLoopCompletesOneResponseThroughSubprocessEngine(t *testing.T) {
	...
	if n := h.count(`SELECT count(*) FROM sessions WHERE  project_id=$1`, h.tenant.Project); n != 1 {
		t.Fatalf("session count = %d, want 1", n)
	}
	if n := h.count(`SELECT count(*) FROM runs WHERE  project_id=$1`, h.tenant.Project); n != 1 {
