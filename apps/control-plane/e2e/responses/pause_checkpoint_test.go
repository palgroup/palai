...
func TestPauseProducesValidCheckpointBeforeComputeRelease(t *testing.T) {
	...
	if n := h.count(`SELECT count(*) FROM tool_calls WHERE run_id=$1 AND organization_id=$2 AND project_id=$3`, f.runID, h.tenant.Project); n != 0 {
	...
	if n := h.count(`SELECT count(*) FROM delivered_messages WHERE run_id=$1 AND organization_id=$2 AND project_id=$3`, f.runID, h.tenant.Project); n != 0 {
...
func TestResumeRestoresFromValidCheckpoint(t *testing.T) {
	...
	if n := h.count(`SELECT count(*) FROM tool_calls WHERE run_id=$1 AND name='recovery.count' AND organization_id=$2 AND project_id=$3`, f.runID, h.tenant.Project); n != 1 {
	...
	if n := h.count(`SELECT count(*) FROM delivered_messages WHERE run_id=$1 AND command_id=$2 AND organization_id=$3 AND project_id=$4`, f.runID, f.msgID, h.tenant.Project); n != 1 {
