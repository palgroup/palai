...
func secretRefTestServer(t *testing.T, verifier middleware.Verifier, api SecretRefAPI) string {
	t.Helper()
	srv := httptest.NewServer(NewRouter(verifier, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, SSEConfig{}, nil, nil, WithSecretRefs(api)))
	t.Cleanup(srv.Close)
	return srv.URL
}
...
func TestSecretRefRoutesUnmountedWhenNil(t *testing.T) {
	srv := httptest.NewServer(NewRouter(scopedVerifier{middleware.Scope{Organization: "org_1"}}, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, SSEConfig{}, nil, nil))
	t.Cleanup(srv.Close)
