...
func mcpTestServer(t *testing.T, reg *fakeMCPRegistry) string {
	t.Helper()
	srv := httptest.NewServer(NewRouter(fakeVerifier{}, nil, nil, nil, nil, nil, nil, nil, nil, nil, reg, SSEConfig{}, nil))
	t.Cleanup(srv.Close)
	return srv.URL
}
...
func TestMCPRoutesUnmountedWhenNil(t *testing.T) {
	srv := httptest.NewServer(NewRouter(fakeVerifier{}, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, SSEConfig{}, nil))
	t.Cleanup(srv.Close)
