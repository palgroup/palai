...
func seedTenantWithKey(t *testing.T, pool *pgxpool.Pool, token string) (coordinator.Tenant, string) {
	...
	exec(t, pool, `INSERT INTO api_keys (id, project_id, principal_id, key_hash) VALUES ($1, $2, $3, $4)`,
...
func TestVerifyAPIKeyResolvesScopeByHash(t *testing.T) {
	...
	if id.Project != tenant.Project || id.Principal != principalID {
		t.Fatalf("identity = %+v, want %s/%s", id, tenant.Project, principalID)
	}
...
func TestAdmitResponseEnforcesPerProjectRunCaps(t *testing.T) {
	...
	assertCount(t, cs.Pool(), 2, `SELECT count(*) FROM runs WHERE  project_id=$1`, tenant.Project)
	...
	exec(t, cs.Pool(), `UPDATE runs SET state='running' WHERE  project_id=$1`, tenant.Project)
	...
	exec(t, cs.Pool(), `UPDATE runs SET state='completed' WHERE project_id=$1 AND state='running' AND id=(SELECT id FROM runs WHERE project_id=$1 AND state='running' ORDER BY id LIMIT 1)`, tenant.Project)
