...
func TestVerifyAPIKeyResolvesScopeByHash(t *testing.T) {
	...
	if id.Organization != tenant.Organization || id.Project != tenant.Project || id.Principal != principalID {
		t.Fatalf("identity = %+v, want %s/%s/%s", id, tenant.Project, principalID)
	}
...
func TestAdmitResponseIsIdempotentAndAtomic(t *testing.T) {
	...
	assertCount(t, cs.Pool(), 1, `SELECT count(*) FROM responses WHERE organization_id=$1 AND project_id=$2`, tenant.Project)
	assertCount(t, cs.Pool(), 1, `SELECT count(*) FROM runs WHERE organization_id=$1 AND project_id=$2`, tenant.Project)
	assertCount(t, cs.Pool(), 1, `SELECT count(*) FROM sessions WHERE organization_id=$1 AND project_id=$2`, tenant.Project)
	assertCount(t, cs.Pool(), 1, `SELECT count(*) FROM events WHERE type='run.queued.v1' AND organization_id=$1 AND project_id=$2`, tenant.Project)
...
func TestAdmitResponseEnforcesPerProjectRunCaps(t *testing.T) {
	...
	assertCount(t, cs.Pool(), 2, `SELECT count(*) FROM runs WHERE state='queued' AND organization_id=$1 AND project_id=$2`, tenant.Project)
	...
	assertCount(t, cs.Pool(), 2, `SELECT count(*) FROM runs WHERE organization_id=$1 AND project_id=$2`, tenant.Project)
	...
	exec(t, cs.Pool(), `UPDATE runs SET state='running' WHERE organization_id=$1 AND project_id=$2`, tenant.Project)
	...
	exec(t, cs.Pool(), `UPDATE runs SET state='completed' WHERE organization_id=$1 AND project_id=$2 AND state='running' AND id=(SELECT id FROM runs WHERE organization_id=$1 AND project_id=$2 AND state='running' LIMIT 1)`, tenant.Project)
