...
var expectedAdminConsoleCatalog = map[string]struct {
	...
	// T8 RAISED THIS FROM 8 TO 9, AND THE +1 IS THE WHOLE OBSERVABILITY HALF (six screens, three routes).
	// That is not a shortfall: the axe half of this case needed NO new reference, because a11y.spec.ts
	// GENERATES its scans and its coverage assertion from lib/routes.ts, so three declared routes became
	// three scans and a ten-route coverage check without a proof leg being retyped. The one genuinely new
	// claim is the read-surface counterpart of visible truncation — a Panel with zero rows states that in a
	// sentence rather than rendering a blank region — asserted over four metering panels on both profiles.
	"CON-002": {"e2e-deterministic", 9, "list truncation is stated in text and continued with the server's cursor, every route lib/routes.ts declares is axe-scanned under the WCAG 2.0+2.1+2.2 tags, an emptied list panel states its emptiness in a sentence rather than going blank, and five divergence-ledger rows the console's evidence rested on were false"},
	...
	"CON-006": {"e2e-deterministic", 10, "a repository binding and an agent lineage are created from screens that read them back, connection_ref and environment are HANDLES chosen from a list with no free-text fallback in either direction, a published revision's model reaches the run's terminal projection while an unpublished one is refused honestly with nothing falling back to a default, and the pin's optional-ness is asserted on the WIRE — plus the component-real leg where the same public HTTP routes end in a real printenv seeing the environment's keys"},
	"CON-007": {"e2e-deterministic", 11, "a shipped runbook's step (c) named a revision id no route returned — it stood three and a half days and was edited three times while wrong; the chain now runs on the public API alone, a foreign tenant's revision is 404, an upstream server's description is inert under attack, and the approval gate defaults ON because Palai cannot classify a write tool and will not guess"},
}
