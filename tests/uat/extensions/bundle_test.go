...
var caseCeilings = map[string][]string{
	"SLK-001": {
		...
		"NO REAL WORKSPACE: every Slack proof in this case runs against a FAKE peer — a fixture that signs the way the verification page documents, and a local HTTP server standing in for slack.com/api. The shipped routes (events + interactivity) and the decision chain are exercised end to end over real Postgres, but nothing here is a receipt from a real workspace: no socket to slack.com exists in this repo's tests. That receipt is §6 leg 1 and is what caps `slack` at preview",
