...
> **Idempotency is principal-scoped (§20.9).** The server dedupes on `(org, project, principal,
> method, route, idempotency_key)`. Every replay of a workflow MUST present the **same API key** — an
> orchestrator fleet whose workers authenticate as *different* principals will replay the same workflow
> id into *distinct* runs (the key matches, but the principal differs). Give the whole fleet one key,
> or scope a key per workflow-owning principal.
