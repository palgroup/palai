...
test("a retry storm under the same workflow id creates exactly ONE run (single retry owner, no multiplication)", async () => {
  ...
  // Non-vacuous: the single key is a real derived key, not an absent/empty header collapsing to one.
  assert.ok([...keys][0]?.startsWith("wf_"), "the shared key is the derived wf_ idempotency key");
});
