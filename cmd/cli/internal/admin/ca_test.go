...
func TestCATrustReachesHTTPSEdge(t *testing.T) {
	...
	if err := Run("pool", []string{"list", "--ca", caFile}, &out, strings.NewReader("")); err != nil {
	...
	out.Reset()
	if err := Run("pool", []string{"list", "--ca", filepath.Join(t.TempDir(), "missing.crt")}, &out, strings.NewReader("")); err == nil || !strings.Contains(err.Error(), "read --ca file") {
	...
	if err := Run("pool", []string{"list", "--ca", empty}, &out, strings.NewReader("")); err == nil || !strings.Contains(err.Error(), "no PEM certificates") {
