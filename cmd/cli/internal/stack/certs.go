...
// carries exactly one DNS name — controllerDNS — because the runner refuses a controller
// certificate that vouches for anyone else. A device reaches it by ADDRESS and verifies it
// by that NAME (`palai enroll --server-name`), so no IP SAN is needed and none is minted:
// every extra identity on a certificate is one more party it can speak for. Keys are 0600;
// the public certs are 0644.
