...
func TestOrderedMigrationsIsContiguousVersionOrder(t *testing.T) {
	...
	if head.Version != 58 || head.Name != "repository_binding_lifecycle" {
		t.Fatalf("chain head = %06d_%s, want 000058_repository_binding_lifecycle", head.Version, head.Name)
	}
	...
	if penultimate.Version != 57 || penultimate.Name != "changeset_ignored_file_count" {
		t.Fatalf("penultimate migration = %06d_%s, want 000057_changeset_ignored_file_count", penultimate.Version, penultimate.Name)
	}
