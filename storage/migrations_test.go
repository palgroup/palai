...
// preflight/journal invariant is anchored: after E17 wave-3 the head is 000039_a2a_remote_agents (E17 T3),
// with 000038_a2a (E17 T2) the penultimate link — strict, no gaps (T1 slack=035 → T4 knowledge=036 →
// T7 queue=037 → T2 a2a=038 → T3 a2a-client=039, merged in the fixed order §1).
func TestOrderedMigrationsIsContiguousVersionOrder(t *testing.T) {
	...
	// E17 T3 A2A client registration is the current chain head (built as 000039 in its worktree, its final
	// number; merged after T2 a2a=038 per the fixed order §1).
	head := migrations[len(migrations)-1]
	if head.Version != 39 || head.Name != "a2a_remote_agents" {
		t.Fatalf("chain head = %06d_%s, want 000039_a2a_remote_agents", head.Version, head.Name)
	}
