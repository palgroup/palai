...
func TestOrderedMigrationsIsContiguousVersionOrder(t *testing.T) {
	...
	if head.Version != 57 || head.Name != "repository_binding_lifecycle" {
		t.Fatalf("chain head = %06d_%s, want 000057_repository_binding_lifecycle", head.Version, head.Name)
	}
	...
	if penultimate.Version != 56 || penultimate.Name != "session_auto_approve" {
		t.Fatalf("penultimate migration = %06d_%s, want 000056_session_auto_approve", penultimate.Version, penultimate.Name)
	}
