...
func TestOrderedMigrationsIsContiguousVersionOrder(t *testing.T) {
	...
	if head.Version != 59 || head.Name != "repository_binding_lifecycle" {
		t.Fatalf("chain head = %06d_%s, want 000059_repository_binding_lifecycle", head.Version, head.Name)
	}
	...
	if penultimate.Version != 58 || penultimate.Name != "job_attempt_error" {
		t.Fatalf("penultimate migration = %06d_%s, want 000058_job_attempt_error", penultimate.Version, penultimate.Name)
	}
