...
// project_scoped_integrity (A.2 Task 6): every uniqueness rule, index and foreign key that names
// organization_id is rebuilt keyed on project alone. It lands BEFORE any writer stops supplying the column
// because a NULL in a unique index is not equal to another NULL — uniqueness would stop existing without
// raising anything. See the migration's own header for the measured surface (45 indexes, 66 composite
// foreign keys) and for why five of those keys are dropped and not replaced.
//
//go:embed migrations/000065_project_scoped_integrity.up.sql
var migrationUp65 string

//go:embed migrations/000065_project_scoped_integrity.down.sql
var migrationDown65 string

// tenant_policy_without_organization (A.2 Task 6): the last twelve row-level-security policies reading
// palai.org_id are rekeyed — three identity tables and four parent-resolving children onto project, four
// tables that have no narrower key onto the installation. See the migration's own header for why
// environments/environment_values/secret_refs/usage_ledger are the one place in this epic where a boundary
// is REMOVED rather than moved, and what that costs an installation hosting two customers.
//
//go:embed migrations/000066_tenant_policy_without_organization.up.sql
var migrationUp66 string

//go:embed migrations/000066_tenant_policy_without_organization.down.sql
var migrationDown66 string

// drop_organizations (A.2 Task 6, last step): organization_id comes off all 86 tables that still carry it
// and the organizations table is dropped, taking with it the one policy 000066 named as its expected
// survivor. See the migration's own header for the measured zeroes that make the DROP COLUMNs safe (no
// index, no foreign key, no view depends on the column) and for the one reader that is not SQL —
// packages/audit.ChainedColumns, whose digest changes with this migration.
//
//go:embed migrations/000067_drop_organizations.up.sql
var migrationUp67 string

//go:embed migrations/000067_drop_organizations.down.sql
var migrationDown67 string
